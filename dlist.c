...
void dlist_print(dlist* list, const char* s){
  printf("%s", s);
  dnode* p = list->head;
...
static void dlist_test_once(dlist* list, int first, int last, int next, void (*fp)(dlist*, int)) {
  for (int i = first; i < last; i += next) { fp(list, i); }
  dlist_print(list, "list is created and is...\n");
  printf("list size is: %zu\n", dlist_size(list));
  printf("list is empty? %s\n", yesorno(dlist_empty(list)));

  int firstadded = first;
  int lastadded = firstadded;
  while (lastadded + next < last) {
    lastadded += next;
  }
